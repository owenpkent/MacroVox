...
- **Keyword boosting was silently ignored** — keywords were parsed and stored in `AppState` but never appended to the Deepgram URLs. Both streaming WebSocket and batch REST paths now include `&keywords=` parameters with proper URL encoding.
