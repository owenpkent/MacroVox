 ...
 *
 * MacroVox ships two dependency graphs: the React renderer (npm) and the
 ...
 *
 *   release/sbom/MacroVox_<ver>_sbom.npm.cyclonedx.json    (renderer, prod deps)
 *   release/sbom/MacroVox_<ver>_sbom.cargo.cyclonedx.json  (Rust backend)
 *
...
if (!/^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/.test(ver)) {
  console.error(`[sbom] refusing to run: package.json version is not strict semver: ${JSON.stringify(ver)}`)
  process.exit(1)
}
const outDir = join(root, 'release', 'sbom')
mkdirSync(outDir, { recursive: true })

const npmOut = join(outDir, `MacroVox_${ver}_sbom.npm.cyclonedx.json`)
const cargoOut = join(outDir, `MacroVox_${ver}_sbom.cargo.cyclonedx.json`)

console.log(`[sbom] MacroVox ${ver}`)
